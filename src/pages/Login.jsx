...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setSuccess('Verification successful. Opening secure vault...');
      setTimeout(() => {
        navigate('/dashboard');
      }, 1000);
    } catch (err) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      await googleLogin();
      setSuccess('Authorized via Google. Synchronizing assets...');
      setTimeout(() => {
        navigate('/dashboard');
      }, 1000);
    } catch (err) {
      ...
      <main className="pt-32 flex items-center justify-center px-4 relative hero-gradient">
        <div className="w-full max-w-md glass-card rounded-2xl p-8 relative z-10 animate-slide-up">
          <div className="text-center mb-8">
            ...
            <h2 className="font-display-lg text-headline-lg font-bold">Secure Portal</h2>
            <p className="text-xs text-on-surface-variant mt-2">Access your AI wealth advisor and premium portfolios.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6 text-left">
            ...
            >
              {loading ? (
                <span className="animate-pulse font-bold">Verifying Credentials...</span>
              ) : (
          ...
          >
            ...
            Access Institutional Google Key
          </button>
