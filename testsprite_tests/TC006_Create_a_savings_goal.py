...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Sign In' button (interactive element [38]) to open the login page so credentials can be entered.
        # button "Sign In"
        elem = page.locator("xpath=/html/body/div/div/header/div[2]/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Fill the email and password fields with the provided credentials and submit the login form by clicking element [272].
        ...
        # -> Retry submitting the login form by clicking the 'Secure Authentication' button (element [272]) to see if authentication succeeds.
        # button "Secure Authentication"
        elem = page.locator("xpath=/html/body/div/div/main/div/form/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the 'Add Goal' button (interactive element [697]) to open the goal creation form.
        # button "add Add Goal"
        elem = page.locator("xpath=/html/body/div/div/main/div/div[4]/div/div[2]/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Fill the goal form (title, target amount, starting asset reserve, deadline, current balance) and submit the form by clicking 'Architect Wealth Target' (element [837]).
        ...
        # -> Click the 'Architect Wealth Target' submit button (element [837]) to create the goal, then verify the new goal appears in the tracker and that the summary reflects the created savings plan.
