...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Sign In' button (element index 288) to open the login page.
        # button "Sign In"
        elem = page.locator("xpath=/html/body/div/div/header/div[2]/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Fill the email field with testsprite@finora-test.com, fill the password field with TestSprite@2026, then submit the sign-in form.
        ...
        # -> Wait briefly for sign-in to complete, then click the 'Dashboard' link (element index 501) to navigate toward Budget Planning.
        # link "Dashboard"
        ...
        # -> Click the 'Select Allocation Pool' dropdown (element index 1456) to reveal category options so a category can be selected before entering a revised target.
        # "-- Choose Category -- Needs (50%) Wants ..."
        elem = page.locator("xpath=/html/body/div/div/main/div/div[3]/div/form/div/select").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Enter a revised target value (50000) into input index 1467 and click the Override Limit button at index 1470 to save the change, then verify the updated budget display.
